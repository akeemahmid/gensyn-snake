  ...
  return (
    ...
    >
      <div
        ...
        tabIndex={0}
        autoFocus
        className="grid grid-rows-20 border-[#fad8d1e6] border-3 outline-none"
      >
        {/* Grid */}
        {Array.from({ length: grizSize }).map((_, y) => (
          <div key={y} className="flex">
            {Array.from({ length: grizSize }).map((_, x) => (
              <div
                key={x}
                className={`w-4 h-4 md:w-5 md:h-5 border  border-[#581000] ${
                  snake.some(
          ...
          <div className="relative z-10 flex flex-col items-center justify-center space-y-6 shadow-[#fad8d1e6] bg-[#1d1d1e] shadow-xs w-auto md:w-[350px] md:h-[340px]  p-6 ">
            ...
            {!showDidYouKnow ? (
              ...
              <div className="w-[300px] md:w-auto text-center">
                ...
                <p className="text-left text-white font-medium  my-[5%]">
                  {randomDescription}
                </p>
                <button
                  className="py-3 px-5 bg-transparent cursor-pointer text-lg font-bold border-[#6e7777] hover:bg-[#fad8d133] border-2 text-white   "
      ...
      {!gameOver && (
        <div className="fixed bottom-0 left-1/2 -translate-x-1/2 flex flex-col items-center gap-2 md:hidden">
          <button
            className="w-16 h-16 rounded-2xl bg-[#581000] border-[#fad8d1e6] border-2 text-white font-bold"
